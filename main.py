...
class ForumApp:
    ...
    def setup_page(self):
        ...
        self.page.padding = ft.padding.only(left=10,right=10,bottom=10, top=25)
        self.page.bgcolor = ft.colors.GREY_100

    def _show_splash_screen(self):
        """
        شاشة ترحيب أنيقة تملأ الشاشة بالكامل أثناء تجهيز التطبيق.
        تُعرض عبر خاصية page.splash المستقلة في Flet (طبقة عائمة فوق كل شيء)
        حتى لا تؤثر إطلاقاً على بقية التصميم الحالي أو بنية الواجهة.
        """
        self.splash_screen = ft.Container(
            expand=True,
            ...
            gradient=ft.LinearGradient(
                ...
                colors=[ft.colors.INDIGO_900, BLUE, PRIMARY],
            ),
            content=ft.Column(
                ...
                controls=[
                    ft.Container(
                        content=ft.Image(src="splash.png", width=100, height=100),
                        width=108,
                        height=108,
                        bgcolor=ft.colors.WHITE,
                        border_radius=100,
                        alignment=ft.alignment.center,
                        border=ft.border.all(1.5, ft.colors.WHITE38),
                    ),
                    ft.Container(height=26),
                    ft.Text(
                        "ملتقى الفكر والدراسات العلمية",
                        size=21,
                    ...
                    ft.Text(
                        "مساحة للحوار والمعرفة والارتقاء الفكري",
                        size=12,
                        color=ft.colors.WHITE70,
                        text_align=ft.TextAlign.CENTER,
                    ),
                    ft.Container(height=45),
                    ft.ProgressRing(width=26, height=26, stroke_width=2.5, color="white"),
                ],
            ),
        )
    ...
    def _open_drawer(self, e):
        """
        يفتح القائمة الجانبية بعد ضبط ارتفاعها على الارتفاع الفعلي الحقيقي للشاشة
        في هذه اللحظة بالذات (self.page.height مضمون أن يكون دقيقاً هنا لأن الصفحة
        عُرضت فعلياً على الجهاز)، بدل الاعتماد على قيمة تقديرية عند بناء الواجهة أول
        مرة، وهو ما كان يسبب زيادة الارتفاع عن المساحة الحقيقية واضطرار المستخدم
        للتمرير لرؤية زر "بلازما" في الأسفل.
        """
        if getattr(self, "_drawer_root_container", None) and self.page.height:
            self._drawer_root_container.height = self.page.height
    ...
    def open_add_topic_dialog(self, e):
        ...
        category_field = ft.Dropdown(
            ...
            options=[
                ...
                ft.dropdown.Option("تقني"), ft.dropdown.Option("إجتماعي"), ft.dropdown.Option("اقتصاد"),
                ft.dropdown.Option("سياسة"), ft.dropdown.Option("قانون"), ft.dropdown.Option("فن"),
                ft.dropdown.Option("تربية"), ft.dropdown.Option("بيئة"), ft.dropdown.Option("فلك"),
                ft.dropdown.Option("إعلام"), ft.dropdown.Option("زراعة"), ft.dropdown.Option("سفر"),
    ...
    def handle_edit_lecture(self, item):
        ...
        summary_field = ft.TextField(
            label="ملخص المحاضرة",
            hint_text="يدعم تنسيق Markdown، مثال:\n# عنوان بارز\n- نقطة أولى\n- نقطة ثانية\n**نص عريض**",
        ...
        def save_edited_lecture(e):
            ...
            updated_fields = {
                ...
                "summary": summary_field.value.strip() if summary_field.value else ""
            }
    ...
    def open_admin_panel(self, e):
                ...
                for item in current_members:
                    ...
                    admin_topics_list.controls.append(
                        ft.Container(
                            content=ft.Column([
                                ft.Row([
                                ft.Text(t_title, size=13, color=BLUE, text_align="right", expand=True),
                                ft.Image(src=f'avatars/{item.get("img")}', width=50,height=50, border_radius=25),
                                ]),
                                ft.Row([admin_btn, del_mem_btn], spacing=0, alignment=ft.MainAxisAlignment.END),
                            ], spacing=0),
                            padding=8,
                            bgcolor=ft.colors.WHITE,
                            border_radius=8,
        ...
        dialog = ft.AlertDialog(
            ...
            content=ft.Container(
                content=ft.Column([
                    ft.Text("يمكنك حذف عضو أو تغيير صلاحيته", size=12, color=ft.colors.GREY_500, width =float("inf"), text_align="center"),
                    admin_topics_list
    ...
    def build_ui(self):
        ...
        plasma_footer = ft.Container(
            content=ft.TextButton(
                "بلازما",
                style=ft.ButtonStyle(color="grey"),
                on_click=lambda _: self.page.launch_url("https://www.facebook.com/plasma.pc"),
            ),
            alignment=ft.alignment.center,
            padding=ft.padding.only(bottom=50),
        )

        # السبب في عدم نزول الزر فعلياً: عناصر self.drawer.controls تُعرض داخل قائمة
        # (ListView) غير محدودة الارتفاع من جهة فلاتر، فلا معنى لـ expand=True هناك مهما
        # فعلنا لأنه لا يوجد ارتفاع محدد ليتمدد ضمنه. الحل: جعل عنصر الـ Drawer الوحيد
        # حاوية بارتفاع صريح يساوي ارتفاع الشاشة الفعلي (self.page.height)، وبداخلها Column
        # يملك الآن سياقاً محدود الارتفاع فعلاً، فيعمل expand=True بشكل صحيح ويُثبَّت
        # الزر في نهاية الشاشة تماماً.
        drawer_height = 500
        self._drawer_root_container = ft.Container(
        ...
        self.drawer.controls = [self._drawer_root_container]

        # تحديث ارتفاع القائمة الجانبية تلقائياً عند تغيّر ارتفاع الشاشة (كتدوير الجهاز)
        # حتى يبقى الزر مثبتاً في آخر الشاشة دائماً.
        def _on_page_resize(e):
            if getattr(self, "_drawer_root_container", None) and self.page.height:
                self._drawer_root_container.height = self.page.height
                self.safe_update()
        self.page.on_resize = _on_page_resize
        ...
        header = ft.Container(
            ...
            on_click=self._open_drawer  # أمر فتح النافذة
        ),
                            #ft.Container(
...
#                            )
    ...
    def open_summary_view(self, lecture_item: dict):
        """
        يفتح صفحة مستقلة تماماً لمعاينة ملخص المحاضرة.
        يدعم تنسيق النصوص عبر صيغة Markdown بدل HTML الخام (وهي الطريقة التي
        يتيحها Flet لعرض عناوين بارزة، قوائم، نص عريض/مائل... إلخ):
          # عنوان رئيسي        -> عنوان بارز
          ## عنوان فرعي
          - عنصر قائمة          -> قائمة نقطية
        ...
        lec_title = lecture_item.get("title", "")
        summary_text = (lecture_item.get("summary") or "").strip()

        summary_body = ft.Markdown(
            summary_text if summary_text else "_لا يوجد ملخص متاح لهذه المحاضرة بعد._",
            selectable=True,
        ...
        summary_view = ft.View(
            ...
            controls=[
                ft.AppBar(
                    title=ft.Text("ملخص المحاضرة", size=15, color="white"),
                    leading=ft.IconButton(ft.icons.ARROW_BACK, on_click=go_back, icon_color="white"),
                    bgcolor=BLUE,
                ),
                ft.ListView(
                    expand=True,
                    ...
                    controls=[
                        ft.Container(
                            ...
                            content=ft.Column(
                                spacing=16,
                                controls=[
                                    ft.Text(lec_title, size=18, weight=ft.FontWeight.BOLD, color=ft.colors.BLUE_900),
    ...
    def open_lecture_detail_view(self, lecture_item: dict):
        ...
        if has_attachment:
            ...
            "الملخص",
                ...
                on_click=lambda e, li=lecture_item: self.open_summary_view(li)
            )
