...
class ForumApp:
    def __init__(self, page: ft.Page):
        ...
        # حالة دورة حياة التطبيق على Android: نُبقي التطبيق آمناً عند الانتقال
        # للخلفية ثم العودة إليه، ولا نسمح لحلقات التحديث بالخلفية بالعمل أثناء الإيقاف.
        self._app_paused = False
        self._app_detached = False
        self._current_dialog = None
        if not hasattr(self, "_root_view"):
            self._root_view = None
        self._install_android_lifecycle_handlers()
    ...
    def clear_session(self):
        ...
        self._root_view.controls.clear()
        self.build_ui()
        self.safe_update()
    ...
    def _install_android_lifecycle_handlers(self):
        """تهيئة أحداث Android Back ودورة حياة التطبيق بشكل مركزي."""
        try:
            self.page.on_app_lifecycle_state_change = self._on_app_lifecycle_change
        except Exception as ex:
            print(f"[lifecycle] تعذر تسجيل حدث دورة الحياة: {ex}")

        # لا نثبت on_view_pop على الجذر؛ عند الجذر نترك Android/Flet يتولى
        # Back الطبيعي (إغلاق/إخفاء التطبيق)، بينما نفعّله فقط عند وجود View فرعية.
        try:
            self.page.on_view_pop = None
        except Exception as ex:
            print(f"[back] تعذر تهيئة حدث زر الرجوع: {ex}")

    def _on_app_lifecycle_change(self, e):
        """إدارة الانتقال للخلفية/العودة دون إعادة إنشاء واجهة التطبيق."""
        try:
            state = e.state
            state_name = getattr(state, "name", str(state))
            print(f"[lifecycle] {state_name}")

            if state_name in ("HIDE", "PAUSE", "INACTIVE"):
                self._app_paused = True
                return

            if state_name in ("RESUME", "RESTART", "SHOW"):
                self._app_paused = False
                # عند العودة لا نعيد بناء الصفحة ولا نعيد تشغيل التطبيق؛
                # نطلب فقط تحديث الواجهة الموجودة بالفعل.
                self.safe_update()
        except Exception as ex:
            print(f"[lifecycle] خطأ: {ex}")

    def _push_view(self, view: ft.View):
        """إضافة View إلى مكدس التنقل بطريقة موحدة وآمنة."""
        try:
            if self._root_view is None:
                return
            # لا نسمح بتكرار نفس الـ View في المكدس.
            if view in self.page.views:
                return
            self.page.views.append(view)
            self.page.on_view_pop = self._on_view_pop
            try:
                self.page.navigate(view.route)
            except Exception:
                try:
                    self.page.go(view.route)
                except Exception:
                    pass
            self.safe_update()
        except Exception as ex:
            print(f"[navigation] push error: {ex}")

    def _pop_current_view(self):
        """إغلاق View الحالية والعودة إلى الجذر مع إيقاف تحديثاتها الخلفية."""
        try:
            ...
            if len(self.page.views) <= 1:
                # عدنا للجذر: اترك Back للجذر تحت إدارة Android/Flet الطبيعية.
                self.page.on_view_pop = None

            # إيقاف جميع حلقات التحديث المرتبطة بصفحة التفاصيل المغلقة.
            for flag in self._active_detail_flags:
                flag["active"] = False
            ...
            print(f"[navigation] pop error: {ex}")
            return False

    def _on_view_pop(self, e):
        """معالج Back المركزي: يغلق آخر View فقط ولا يعبث بالـ root View."""
        try:
            if len(self.page.views) > 1:
                self._pop_current_view()
                return
            # عند الجذر لا نحذف root view ولا نفرغ الصفحة.
            # ترك الجذر سليماً يمنع تلف Navigator عند إعادة فتح التطبيق على Android.
            return
        except Exception as ex:
            print(f"[back] خطأ أثناء معالجة الرجوع: {ex}")

    def show_dialog(self, dialog: ft.AlertDialog):
        """عرض AlertDialog بطريقة متوافقة مع إصدارات Flet المختلفة."""
        ...
        try:
            ...
            print(f"[show_dialog] خطأ في page.show_dialog: {ex}")

        # توافق مع الإصدارات الأقدم التي لا توفر page.show_dialog().
        try:
            ...
            print(f"[show_dialog] خطأ في fallback: {ex}")

    def close_dialog(self):
        """إغلاق آخر AlertDialog مفتوح."""
        try:
            ...
            print(f"[close_dialog] خطأ في page.pop_dialog: {ex}")
        ...
        try:
            ...
            print(f"[close_dialog] خطأ في fallback: {ex}")
...
#                            )
        ...
        self._root_view.controls.extend([header, self.upcoming_banner, self.search_field, tabs])
        self.load_all_data()
    ...
    def _make_comments_loader(self, *, comments_column, last_update_text, table_name, id_field,
                               ...
                               date_color=ft.colors.GREY_600, on_change=None):
        """
    ...
    def open_summary_view(self):
        summary_view = ft.View(
            ...
            controls=[
                ft.AppBar(
                    title=ft.Text("الملخص", color="white"),
                    bgcolor=BLUE,
                    leading=ft.IconButton(
                        ft.icons.ARROW_BACK,
                        icon_color="white",
                        on_click=lambda e: self._pop_current_view(),
                    ),
                ),
                ft.Container(
                    expand=True,
                    alignment=ft.alignment.center,
                    content=ft.Text("الملخص", size=20, color=BLUE),
                ),
            ],
        )
        self._push_view(summary_view)
    def open_lecture_detail_view(self, lecture_item: dict):
        ...
        def auto_refresh_comments():
            ...
            while view_state["active"] and not self._app_detached:
                time.sleep(6)
                if not view_state["active"] or self._app_detached:
                    break
                if self._app_paused:
                    continue
                try:
...
def main(page: ft.Page):
    ...
    # -- شاشة ترحيب (Splash Screen) تظهر فوراً عند فتح التطبيق باستخدام assets/splash.png --
    # ملاحظة: عند بناء APK فإن Flet ينسخ splash.png تلقائياً كشاشة إقلاع أصلية (Native
    # Splash) تظهر قبل تشغيل بايثون نفسه، وهذه الشاشة هنا تكملها داخل التطبيق أثناء
    # تحميل الواجهة الفعلية حتى لا تظهر شاشة بيضاء/فارغة بينهما.
    ...
    splash_screen = ft.Container(
        ...
        content=ft.Column(
            [
                ft.Image(src="splash.png", width=200, fit=ft.ImageFit.CONTAIN),
                ft.Container(height=24),
                ft.ProgressRing(width=20, height=20, stroke_width=2, color=ft.colors.INDIGO_400),
            ],
            spacing=0,
            alignment=ft.MainAxisAlignment.CENTER,
            horizontal_alignment=ft.CrossAxisAlignment.CENTER,
    ...
    # مهم على Android: لا نُنشئ ForumApp من Thread مستقل ثم نربطه بـ Page.
    # Flet يدير دورة حياة Page/Flutter من خيطه الخاص، وإنشاء الواجهة من خارج
    # دورة حياة الصفحة قد يزيد احتمالات عدم استقرار إعادة الفتح بعد زر Back.
    time.sleep(0.2)
    page.controls.clear()
    app = ForumApp.__new__(ForumApp)
    app._root_view = ft.View("/", controls=[], can_pop=False)
    page.views.clear()
    page.views.append(app._root_view)
    # إعادة تشغيل التهيئة الطبيعية مع root View موجودة مسبقاً.
    ForumApp.__init__(app, page)
    app.safe_update()
