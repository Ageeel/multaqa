...
class ForumApp:
    def __init__(self, page: ft.Page):
        ...
        # آخر AlertDialog تم عرضه عبر show_dialog، تُستخدم لإغلاقه لاحقاً عبر close_dialog
        # (بديل لأسلوب page.dialog القديم الذي يسبب مشكلة عدم إغلاق النوافذ بعد بناء APK)
    ...
    def show_dialog(self, dialog: ft.AlertDialog):
        """
        يعرض AlertDialog بطريقة متوافقة مع تطبيقات الأندرويد بعد بناء APK.

        السبب: أسلوب "page.dialog = dialog" القديم كان يُنشئ ويُزيل مرجع النافذة من
        الصفحة في كل مرة، وهذا تحديداً هو ما يسبب مشكلة "عدم إغلاق النوافذ" الشهيرة
        في Flet 0.19.0 بعد تصدير APK (تعمل بشكل سليم أثناء التطوير على الحاسوب لكن
        تتعطل على الجهاز الفعلي). البديل الموثوق: نضيف النافذة مرة واحدة إلى
        page.overlay ثم نتحكم بظهورها/إخفائها فقط عبر خاصية open + تحديث الصفحة،
        دون إعادة تعيين page.dialog في كل مرة.
        """
    ...
    def close_dialog(self):
        """يغلق أي AlertDialog معروض حالياً عبر show_dialog."""
...
#                            )
    ...
    def _prepare_root_view_for_android(self):
        """منع Android/Flet من محاولة حذف View الجذر عند الضغط على Back."""
        try:
            if self.page.views:
                self.page.views[0].can_pop = False
        except Exception as ex:
            print(f"[root_view] {ex}")

    def _pop_detail_view(self, view_state=None):
        """إغلاق View التفاصيل فقط، مع إبقاء View الجذر دائماً."""
        try:
            if view_state is not None:
                view_state["active"] = False
            if len(self.page.views) > 1:
                self.page.views.pop()
                self._prepare_root_view_for_android()
                self.safe_update()
        except Exception as ex:
            print(f"[pop_detail_view] {ex}")
    ...
    def open_lecture_detail_view(self, lecture_item: dict):
        ...
        def go_back(e):
            self._pop_detail_view(view_state)
...
def main(page: ft.Page):
    ...
    def handle_android_view_pop(e):
        """Back: أغلق View الحالية فقط، مع إبقاء View الجذر وعدم ترك Threads تعمل."""
        try:
            if len(page.views) > 1:
                for flag in getattr(app_ref[0], "_active_detail_flags", []):
                    flag["active"] = False
                getattr(app_ref[0], "_active_detail_flags", []).clear()
                page.views.pop()
                page.update()
            # عند الجذر: لا نحذف root لأن root.can_pop=False.
        except Exception as ex:
            print(f"[android_back] {ex}")

    page.on_view_pop = handle_android_view_pop

    def start_app():
        # نبني التطبيق الفعلي (قراءة الكاش المحلي + بناء الواجهة) في خيط منفصل حتى لا
        # نُجمّد شاشة الترحيب، مع مهلة قصيرة (نصف ثانية) لعرضها بشكل مريح للعين قبل
        # الانتقال المباشر لواجهة المنتدى.
        time.sleep(0.5)
        page.controls.clear()
        app = ForumApp(page)
        app_ref[0] = app
        try:
            app._prepare_root_view_for_android()
        except Exception as ex:
            print(f"[android_root] {ex}")
