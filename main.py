...
class ForumApp:
    def __init__(self, page: ft.Page):
        ...
        # شاشة ترحيب أنيقة تملأ كامل الشاشة تُعرض فوراً، قبل أي عملية شبكة/تحميل
        # بطيئة (فحص Supabase، تحميل الجلسة، بناء الواجهة)، حتى لا يرى المستخدم
        # شاشة بيضاء أثناء هذه العمليات. تُزال تلقائياً في نهاية build_ui().
    ...
    def show_splash_screen(self):
        """
        شاشة ترحيب تملأ كامل مساحة الشاشة دون أي هوامش، بشعار التطبيق وسط
        بطاقة دائرية بيضاء بارزة (ظل) على خلفية متدرّجة بألوان التطبيق، مع
        عنوان التطبيق واسمه الفرعي ومؤشر تحميل دائري، وحركة دخول ناعمة
        (تكبير + تلاشي) بعد عرض العنصر مباشرة كي لا تبدو الشاشة جامدة.
        """
        self.page.padding = 0
        self.page.bgcolor = "#0C1642"
        ...
        logo_badge = ft.Container(
            ...
            shadow=ft.BoxShadow(spread_radius=2, blur_radius=28, color="#00000050", offset=ft.Offset(0, 10)),
            content=ft.Image(src="splashe.png", width=96, height=96, fit=ft.ImageFit.CONTAIN),
            scale=0.85,
            animate_scale=600,
        )
        app_title = ft.Text(
            "ملتقى الفكر", size=30, weight=ft.FontWeight.BOLD, color=ft.colors.WHITE,
            font_family="font", opacity=0, animate_opacity=500,
        )
        app_subtitle = ft.Text(
            "منصة الدراسات والمقترحات والبحوث العلمية",
            size=13, color=ft.colors.WHITE70, text_align=ft.TextAlign.CENTER,
            font_family="font", opacity=0, animate_opacity=500,
        )
        loader = ft.ProgressRing(
            width=28, height=28, stroke_width=3, color=ft.colors.WHITE,
        ...
        splash = ft.Container(
            ...
            gradient=ft.LinearGradient(
                ...
                colors=["#0B1440", "#16267A", ft.colors.INDIGO_900, "#1E3FA8"],
            ),
            content=ft.Column(
...
#                            )
        ...
        # إزالة شاشة الترحيب فور اكتمال بناء الواجهة الفعلية
        self.page.controls.clear()
