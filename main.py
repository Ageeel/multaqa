...
from cache.default_data import *
...
class ForumApp:
    def __init__(self, page: ft.Page):
        ...
        # يُفضّل ضبط هذه القيم كمتغيرات بيئة (Environment Variables) بدل تركها ظاهرة
        # مباشرة بالكود المصدري. القيم هنا تُستخدم فقط كاحتياط إن لم تكن المتغيرات مضبوطة،
        # حتى لا يتعطل التطبيق أثناء التطوير.
        self.SUPABASE_URL = os.environ.get("SUPABASE_URL", "https://fbtrkfrzndfqfffixfzi.supabase.co")
        self.SUPABASE_KEY = os.environ.get("SUPABASE_KEY", "sb_publishable_9fzWVetwvxbXptXwZihR5g__RHnELA6")
        ...
        # تخزين مؤقت خفيف لبيانات الأعضاء في الذاكرة لتقليل عدد الـ Threads التي كانت
        # تُفتح مع كل استدعاء لـ fetch_with_cache (مثلاً is_current_user_admin كانت
        # تُستدعى في كل بناء واجهة وتفتح Thread جديد في كل مرة)
        self._members_cache = None
        self._members_cache_time = 0
        self._members_cache_ttl = 15  # ثانية

        # تتبّع حلقات التحديث التلقائي النشطة لصفحات التفاصيل (محاضرة/مقترح) حتى نوقف
        # أي حلقة سابقة عند فتح صفحة تفاصيل جديدة بدل تركها تعمل بالخلفية بلا داعٍ
        self._active_detail_flags = []
        ...
        # مجلد تخزين الكاش محلياً. نستخدم _get_writable_storage_dir() بدل مسار نسبي بسيط
        # لأن المسار النسبي البسيط ("cache") قد يُشير لمجلد للقراءة فقط داخل حزمة APK
        # المبنية على أندرويد (خلافاً لبيئة Pydroid حيث يعمل بلا مشاكل) - هذا على الأغلب
        # هو سبب ظهور شاشة بيضاء فقط عند فتح التطبيق المبني كـ APK، لأن محاولة إنشاء هذا
        # المجلد كانت تفشل بصمت (استثناء غير معالَج) قبل رسم أي عنصر بالواجهة إطلاقاً.
        self.storage_dir = self._get_writable_storage_dir()
        ...
        self.default_members = [
            {"id": 18, "member": "محي الدين الهادي", "pwd": "123", "img": "18.png", "birth_date": "1996-01-01", "is_admin": True}
        ]

        self.default_lectures = lectures
    ...
    def hash_password(raw_pwd: str) -> str:
        """تشفير كلمة السر (SHA-256) بدل تخزينها كنص عادي في القاعدة/الكاش."""
        return hashlib.sha256(raw_pwd.strip().encode("utf-8")).hexdigest()

    def get_members_cached(self, force_refresh: bool = False):
        """
        نسخة مخفّفة من fetch_with_cache مخصّصة لبيانات الأعضاء: تعيد نفس البيانات من
        الذاكرة إن كانت حديثة (خلال آخر self._members_cache_ttl ثانية) بدل فتح Thread
        جديد مع كل استدعاء، لأن is_current_user_admin وغيرها كانت تناديها بكثرة.
        """
        now = time.time()
        if not force_refresh and self._members_cache is not None and (now - self._members_cache_time) < self._members_cache_ttl:
            return self._members_cache
        self._members_cache = self.fetch_with_cache("members", self.default_members)
        self._members_cache_time = now
        return self._members_cache
    ...
    def is_current_user_admin(self) -> bool:
        ...
        if not self.current_user_id:
            return False
        members_data = self.get_members_cached()
    ...
    def setup_page(self):
        ...
        self.page.padding = ft.padding.only(left=10,right=10,bottom=10, top=40)
    ...
    def _get_writable_storage_dir(self):
        """
        يحدد مجلداً قابلاً للكتابة فعلياً لتخزين الكاش وصور الأعضاء محلياً، بنظام احتياطي
        متدرج: أولاً FLET_APP_STORAGE_DATA إن كانت متوفرة (مجلد بيانات دائم يوفره Flet
        نفسه)، ثم المجلد الحالي (يعمل غالباً في Pydroid والمعاينة المباشرة)، وأخيراً مجلد
        النظام المؤقت (tempfile) كحل أخير مضمون الكتابة على أي منصة. أي فشل هنا لن يُسقط
        التطبيق، بل ينتقل للخيار التالي.
        """
        base = os.environ.get("FLET_APP_STORAGE_DATA") or os.getcwd()
        try:
            test_dir = os.path.join(base, "app_storage")
            os.makedirs(test_dir, exist_ok=True)
            return test_dir
        except Exception:
            import tempfile
            fallback_dir = os.path.join(tempfile.gettempdir(), "app_storage")
            os.makedirs(fallback_dir, exist_ok=True)
            return fallback_dir

    def open_dialog(self, dialog):
        """
        يفتح نافذة AlertDialog. بالإضافة لـ page.dialog التقليدية، نُسجّل النافذة صراحة
        كعنصر ضمن page.controls (عبر page.add) - لأن الاعتماد على page.dialog وحدها
        معروف بأنه لا يُغلق بشكل موثوق في بناء APK النهائي (Release) على أندرويد رغم
        عمله بشكل طبيعي في المعاينة المباشرة (flet run). هذا هو الحل المؤكد فعلياً من
        تقرير الخلل الرسمي لـ Flet (flet-dev/flet#3664) لهذه المشكلة تحديداً - إضافة
        النافذة لعناصر الصفحة مباشرة بدل page.overlay التي لا تحل المشكلة فعلياً.
        """
    ...
    def close_dialog(self, dialog=None):
        """يغلق النافذة الحالية (أو نافذة محددة). نُبقيها ضمن page.controls (بدل إزالتها)
        مطابقةً للنمط الموثّق في حل مشكلة Flet، ونعتمد فقط على تبديل خاصية open."""
        d = dialog or self.page.dialog
        if d:
            d.open = False
        self.safe_update()
    ...
    # الأعمدة الحقيقية الموجودة فعلياً بجدول lectures في سيوبابيز (كما تظهر بلوحة التحكم).
    # أي حقل محلي بالتطبيق غير مذكور هنا (مثل completed_at) سيُستبعد تلقائياً قبل الإرسال
    # للخادم، بدل أن يتسبب في رفض الطلب بالكامل بصمت بسبب "عمود غير موجود".
    LECTURE_DB_COLUMNS = {
        "id", "title", "speaker", "speaker_image", "date", "type",
        "finished", "votes", "suggested_by", "locked", "summary",
    ...
    def _lecture_to_db_payload(self, data: dict) -> dict:
        ...
        if "member_img" in payload:
            payload["speaker_image"] = payload.pop("member_img")
        # حالياً عمود completed_at غير موجود بالجدول الحقيقي (راجع ملاحظة SQL أدناه لإضافته)،
        # لذا نستبعد أي حقل لا يقابل عموداً حقيقياً حتى لا يفشل الطلب كاملاً بصمت
        dropped = {k: v for k, v in payload.items() if k not in self.LECTURE_DB_COLUMNS}
        if dropped:
            print(f"[_lecture_to_db_payload] تم تجاهل حقول لا تقابل أعمدة حقيقية بجدول lectures: {dropped}")
        payload = {k: v for k, v in payload.items() if k in self.LECTURE_DB_COLUMNS}
        return payload
    ...
    def get_cached_member_image(self, img_column_value):
        ...
        # نخزّن الصور المُنزَّلة ضمن self.storage_dir القابل للكتابة، وليس داخل مجلد
        # "assets" لأن هذا المجلد يُجمَّع للقراءة فقط داخل حزمة APK النهائية على أندرويد،
        # ومحاولة الكتابة فيه تفشل بصمت وتمنع رسم الواجهة (شاشة بيضاء عند التشغيل).
        avatars_cache_dir = os.path.join(self.storage_dir, "avatars")
        try:
    ...
    def open_login_dialog(self, e):
        ...
        def verify_login(e):
            ...
            entered_hash = self.hash_password(entered_pwd)
            matched_member = next(
                (m for m in members_data if str(m.get("pwd")).strip() in (entered_hash, entered_pwd)),
                None
            )

            if matched_member:
                # ترقية كلمات السر القديمة المخزّنة كنص عادي إلى صيغة مشفّرة تلقائياً
                # عند أول تسجيل دخول ناجح بها، دون إزعاج المستخدم بأي خطوة إضافية
                if str(matched_member.get("pwd")).strip() == entered_pwd and entered_pwd != entered_hash:
                    matched_member["pwd"] = entered_hash
                    self.set_cached_data_to_file("members", members_data)
                    self._members_cache = None
                    if self.db.is_connected:
                        try:
                            self.db.update_data("members", matched_member.get("id"), {"pwd": entered_hash})
                        except Exception:
                            pass
        ...
        dialog = ft.AlertDialog(
            ...
            actions=[
                ft.ElevatedButton("إلغاء", height=35, on_click=lambda e: self.close_dialog()),
    ...
    def open_edit_profile_dialog(self, e):
        ...
        pwd_field = ft.TextField(label="كلمة السر الجديدة (اتركها فارغة للإبقاء عليها)", value="", password=True, can_reveal_password=True, text_align=ft.TextAlign.RIGHT, border_radius=8, border_color=BLUE, color=BLUE, height=45, content_padding=10)
        ...
        def save_changes(e):
            ...
            if not new_name:
                error_text.value = "الاسم حقل إجباري."
                self.safe_update()
                return

            updated_fields = {"member": new_name, "birth_date": new_birth}
            if new_pwd:
                updated_fields["pwd"] = self.hash_password(new_pwd)
    ...
    def open_add_topic_dialog(self, e):
        ...
        def save_new_topic(e):
            ...
            self.close_dialog()
            self.page.show_snack_bar(ft.SnackBar(content=ft.Text("تم إضافة مقترحك بنجاح!", text_align="center"), bgcolor=DONE))
            self.load_topics()
    ...
    def open_add_member_dialog(self, e):
        if not self.is_current_user_admin():
            self.page.show_snack_bar(ft.SnackBar(content=ft.Text("عفواً، هذه الصلاحية للمدراء فقط", text_align="center"), bgcolor=ft.colors.RED_400))
            return
        ...
        def save_new_member(e):
            ...
            new_member = {
                "id": new_id,
                "member": new_name,
                "pwd": self.hash_password(new_pwd),
                "img": "default.png",
                "is_admin": False
            }
    ...
    def handle_delete_lecture(self, item):
        ...
        # نحذف كل التعليقات المرتبطة بهذه المحاضرة محلياً حتى لا تبقى تعليقات يتيمة
        lecture_comments = self.get_cached_data_from_file("lecture_comments", [])
        lecture_comments = [c for c in lecture_comments if str(c.get("lecture_id")) != str(lec_id)]
        self.set_cached_data_to_file("lecture_comments", lecture_comments)

        if self.db.is_connected:
            try:
                self.db.delete_data("lectures", lec_id)
                self.db.delete_data_where("lecture_comments", "lecture_id", lec_id)
            except Exception:
                pass
        self.load_all_data()
        self.page.show_snack_bar(ft.SnackBar(content=ft.Text("تم حذف المحاضرة وتعليقاتها بنجاح", text_align="center"), bgcolor=DONE))
    ...
    def handle_edit_lecture(self, item):
        ...
        def save_edited_lecture(e):
            ...
            if self.db.is_connected:
                try:
                    self.db.update_data("lectures", item.get("id"), self._lecture_to_db_payload(updated_fields))
                except Exception:
                    pass
        ...
        dialog = ft.AlertDialog(
            ...
            actions=[
                ft.ElevatedButton("إلغاء", on_click=lambda e: self.close_dialog()),
    ...
    def handle_assign_lecturer(self, item):
        ...
        def save_assignment(e):
            ...
            if not update_ok:
                self.close_dialog()
                self.page.show_snack_bar(
                    ft.SnackBar(content=ft.Text("تعذر حفظ التعيين على الخادم، يرجى المحاولة لاحقاً", text_align="center"), bgcolor=ft.colors.RED_400)
                )
                return
    ...
    def handle_delete_topic(self, item):
        ...
        # نحذف كل التعليقات المرتبطة بهذا المقترح محلياً حتى لا تبقى تعليقات يتيمة
        topic_comments = self.get_cached_data_from_file("topic_comments", [])
        topic_comments = [c for c in topic_comments if str(c.get("topic_id")) != str(top_id)]
        self.set_cached_data_to_file("topic_comments", topic_comments)

        if self.db.is_connected:
            try:
                self.db.delete_data("topics", top_id)
                self.db.delete_data_where("topic_comments", "topic_id", top_id)
            except Exception:
                pass
        self.load_all_data()
        self.page.show_snack_bar(ft.SnackBar(content=ft.Text("تم حذف المقترح وتعليقاته بنجاح", text_align="center"), bgcolor=DONE))
    ...
    def handle_accept_topic(self, item):
        ...
        new_lec = {
            ...
            "member_id": None,  # لا يوجد محاضر معيّن بعد؛ None لتفادي مخالفة قيد المفتاح الأجنبي مع members
            "speaker": "لم يحدد",
            "speaker_image": "",
        ...
        insert_ok = True
        if self.db.is_connected:
            insert_ok = self.db.insert_data("lectures", self._lecture_to_db_payload(new_lec))

        if not insert_ok:
            self.page.show_snack_bar(
                ft.SnackBar(content=ft.Text("تعذر حفظ المحاضرة على الخادم، يرجى المحاولة لاحقاً", text_align="center"), bgcolor=ft.colors.RED_400)
            )
            return

        # لا نُعلّم المقترح كمقبول ولا نضيف المحاضرة محلياً إلا بعد التأكد من نجاح الحفظ
        # على الخادم (أو غياب الاتصال أصلاً)، حتى لا تختفي بيانات لم تُحفظ فعلياً
        item["accepted"] = "true"
    ...
    def open_admin_panel(self, e):
        if not self.is_current_user_admin():
            self.page.show_snack_bar(ft.SnackBar(content=ft.Text("عفواً، الوصول للوحة التحكم للمدراء فقط", text_align="center"), bgcolor=ft.colors.RED_400))
            return
                ...
                for item in current_members:
                    ...
                    admin_topics_list.controls.append(
                        ft.Container(
                            content=ft.Column([
                                ft.Row([
                                ft.Image(src=self.get_cached_member_image(item.get("img")), width=50,height=50, border_radius=25),
                                ft.Text(t_title, size=13, color=BLUE, expand=True, text_align=ft.TextAlign.RIGHT)]),
        ...
        dialog = ft.AlertDialog(
            ...
            actions=[
                ft.ElevatedButton("إغلاق", height=35, on_click=lambda e: self.close_dialog())
            ],
            actions_alignment=ft.MainAxisAlignment.CENTER,
        )
...
#                            )
        ...
        members_header = ft.Container(content=ft.Row([
            ft.Text(f"أعضاء الملتقى ( {len(total_members)} )", color=ft.colors.GREY_500)
        ...
        # نُغلّف الترويسة العلوية فقط بـ SafeArea (وليس كامل الصفحة تفادياً لخلل معروف عند
        # دمج SafeArea مع Tabs في Flet) لتفادي تداخلها مع شريط حالة الهاتف/النوتش بشكل
        # ديناميكي حسب كل جهاز، إضافة للحشو الثابت المضبوط أعلاه كطبقة حماية إضافية
        self.page.add(ft.SafeArea(content=header), self.upcoming_banner, self.search_field, tabs)
        self.load_all_data()
    ...
    def handle_vote(self, item: dict, table_name: str):
        ...
        if table_name == "lectures":
            ...
            if is_locked and not is_admin:
                self.page.show_snack_bar(ft.SnackBar(content=ft.Text("هذه المحاضرة مقفلة، لا يمكن التفاعل أو التصويت عليها", text_align="center"), bgcolor=ft.colors.RED_400))
                return
    ...
    def _loading_placeholder(self, message: str):
        """عنصر مؤقت يظهر فقط عند أول تشغيل (قبل توفر أي نسخة محلية من البيانات)،
        بدل ظهور رسالة 'لا توجد نتائج' بشكل مضلل ريثما تصل البيانات من الخادم."""
        return ft.Container(
            content=ft.Column(
                [ft.ProgressRing(width=22, height=22, stroke_width=3, color=BLUE), ft.Text(message, color=ft.colors.GREY_600, size=12)],
                horizontal_alignment=ft.CrossAxisAlignment.CENTER,
                spacing=10
            ),
            alignment=ft.alignment.center,
    ...
    def _make_comments_loader(self, *, comments_column, last_update_text, table_name, id_field,
                               id_value, default_comments, is_admin, show_rating=False,
                               date_color=ft.colors.GREY_600, on_change=None, page_size=5):
        """
        تُنشئ دالة تحميل/عرض تعليقات (مع التعديل والحذف) قابلة لإعادة الاستخدام، بدل
        تكرار نفس منطق العرض بين صفحة تفاصيل المحاضرة وصفحة تفاصيل المقترح كما كان سابقاً.

        table_name: اسم جدول التعليقات ("lecture_comments" أو "topic_comments")
        id_field:   اسم حقل الربط بالعنصر الأب ("lecture_id" أو "topic_id")
        show_rating: تعرض قيمة التقييم أسفل التعليق (خاص بتعليقات المحاضرات فقط)
        on_change:  دالة اختيارية تُستدعى بعد كل تعديل/حذف (مثل تحديث عداد تعليقات المقترحات بالقائمة)
        page_size:  الحد الأقصى لعدد التعليقات المعروضة دفعة واحدة، مع زر "عرض المزيد" للباقي
        """
        visible_state = {"limit": page_size}

        def load_fn():
            comments_data = self.fetch_with_cache(table_name, default_comments)
            filtered_comments = [c for c in comments_data if str(c.get(id_field)) == str(id_value)]
            members_data = self.get_members_cached()

            comments_column.controls.clear()
            last_update_text.value = f"آخر تحديث: {datetime.now().strftime('%H:%M:%S')}"
            ...
            if not filtered_comments:
                ...
                visible_comments = filtered_comments[:visible_state["limit"]]
                for c in visible_comments:
                    ...
                    avatar_src = self.get_cached_member_image(img_val) if author_name not in "بلازما للبرمجيات" else "avatars/plasma.png"
                    ...
                    def edit_comm(comment_item=c):
                        ...
                        def save_edit(e):
                            ...
                            if self.db.is_connected and "id" in comment_item:
                                try:
                                    self.db.update_data(table_name, comment_item.get("id"), {"content": comment_item["content"]})
                                except Exception:
                                    pass
                            self.set_cached_data_to_file(table_name, comments_data)
                            self.close_dialog()
                            load_fn()
                            if on_change:
                                on_change()
                            self.page.show_snack_bar(ft.SnackBar(content=ft.Text("تم تعديل التعليق بنجاح", text_align="center"), bgcolor=DONE))
                        edit_comm_dialog = ft.AlertDialog(
                        ...
                        self.open_dialog(edit_comm_dialog)
                    ...
                    def delete_comm(comment_item=c):
                        ...
                        if self.db.is_connected and "id" in comment_item:
                            try:
                                self.db.delete_data(table_name, comment_item.get("id"))
                            except Exception:
                                pass
                        self.set_cached_data_to_file(table_name, comments_data)
                        load_fn()
                    ...
                    rating_row = ft.Text(f"التقييم: {c.get('rating', '')}", size=11, color=ft.colors.AMBER_800) if (show_rating and c.get("rating")) else ft.Container()
                        ...
                        ft.Container(
                            ...
                            content=ft.Column([
                                ft.Row([
                                    ...
                                    ft.Row([ft.Text(c.get("date", ""), size=10, color=date_color), actions_row], spacing=0)
                                ], alignment=ft.MainAxisAlignment.SPACE_BETWEEN),
                ...
                remaining = len(filtered_comments) - len(visible_comments)
                if remaining > 0:
                    def show_more_comments(e):
                        visible_state["limit"] += page_size
                        load_fn()
                    ...
                    comments_column.controls.append(
                        ft.Container(
                            content=ft.TextButton(
                                f"عرض {min(remaining, page_size)} تعليقات إضافية (متبقي {remaining})",
                                on_click=show_more_comments
                            ),
                            alignment=ft.alignment.center,
                            padding=5
                        )
            ...
            try:
                ...
                self.safe_ctrl_update(last_update_text)
            except Exception:
                self.safe_update()
    ...
    def load_speakers(self):
        ...
        if self.get_cached_data_from_file("members", None) is None:
            self.speakers_list.controls.clear()
            self.speakers_list.controls.append(self._loading_placeholder("جاري تحميل الأعضاء..."))
            self.safe_update()
    ...
    def load_lectures(self):
        def render_lectures_list(data):
            ...
            all_lecture_comments = self.get_cached_data_from_file("lecture_comments", [])
            is_admin = self.is_current_user_admin()

            for item in current_page_items:            
                item["total"] = len(members_counter)
                item["comments_count"] = len([c for c in all_lecture_comments if str(c.get("lecture_id")) == str(item.get("id"))])
                finished_val = str(item.get("finished", "false")).lower()
        ...
        if self.get_cached_data_from_file("lectures", None) is None:
            self.lectures_list.controls.clear()
            self.lectures_list.controls.append(self._loading_placeholder("جاري تحميل المحاضرات..."))
            self.safe_update()
    ...
    def open_lecture_detail_view(self, lecture_item: dict):
        ...
        # نوقف أي حلقات تحديث سابقة (لصفحات تفاصيل أخرى) بدل تركها تعمل بالخلفية بلا داعٍ
        for f in self._active_detail_flags:
            f["active"] = False
        self._active_detail_flags.clear()
        view_state = {"active": True}
        self._active_detail_flags.append(view_state)
        ...
        last_update_text = ft.Text("", size=10, color=ft.colors.GREY_500)

        load_comments = self._make_comments_loader(
            comments_column=comments_column,
            last_update_text=last_update_text,
            table_name="lecture_comments",
            id_field="lecture_id",
            id_value=lec_id,
            default_comments=[{"id": 1, "lecture_id": lec_id, "author": "عضو", "content": "محتوى قيم جداً.", "date": "2026-08-19", "rating": "ممتاز"}],
            is_admin=is_admin,
            show_rating=True,
        )
                ...
                ft.ListView([
                    ft.Container(
                        ...
                        content=ft.Column([
                            ...
                            ft.Row([ft.Text("التقييم والتعليقات", size=13, color=BLUE), last_update_text], alignment=ft.MainAxisAlignment.SPACE_BETWEEN),
    ...
    def load_topics(self):
        def render_topics_list(topics):
            ...
            topics = sorted(topics, key=lambda x: x.get("id", 0), reverse=True)
            ...
            for item in topics:
                ...
                def open_topic_detail(topic_item=item):
                    ...
                    comment_input = ft.TextField(label="أضف تعليقاً على المقترح...", border_radius=8, border_color=BLUE, text_align=ft.TextAlign.RIGHT, label_style=ft.TextStyle(color=BLUE, size=13), color=BLUE, multiline=True, min_lines=1, max_lines=3, content_padding=20)
                    comments_column = ft.Column(spacing=8)
                    last_update_text = ft.Text("", size=10, color=ft.colors.GREY_500)
                    ...
                    load_topic_comments = self._make_comments_loader(
                        comments_column=comments_column,
                        last_update_text=last_update_text,
                        table_name="topic_comments",
                        id_field="topic_id",
                        id_value=top_id,
                        default_comments=default_topic_comments,
                        is_admin=is_admin,
                        show_rating=False,
                        date_color=ft.colors.GREY_500,
                        on_change=self.load_topics,
                    )
                                ...
                                ft.Container(
                                    ...
                                    content=ft.Column([
                                        ft.Column([
                                            ...
                                            ft.Row([ft.Text(f"التاريخ: {topic_item.get('date')} - ", size=12, color=ft.colors.BLUE_400),
                                        ...
                                        ft.Row([ft.Text("نقاشات الأعضاء حول المقترح", size=13, color=BLUE), last_update_text], alignment=ft.MainAxisAlignment.SPACE_BETWEEN),
                                        ft.Divider(height=1, color=ft.colors.GREY_200),
                                        ft.Container(height=10),
                                        comments_column
                                    ], spacing=12)
        ...
        if self.get_cached_data_from_file("topics", None) is None:
            self.topics_list.controls.clear()
            self.topics_list.controls.append(self._loading_placeholder("جاري تحميل المقترحات..."))
            self.safe_update()
