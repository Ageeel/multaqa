...
class ForumApp:
    ...
    def _show_splash_screen(self):
        ...
        self.splash_screen = ft.Container(
            ...
            content=ft.Column(
                ...
                controls=[
                    ft.Container(
                        content=ft.Icon(ft.icons.MENU_BOOK_ROUNDED, size=54, color="white"),
                        width=108,
                        height=108,
    ...
    def _open_drawer(self, e):
        ...
        if getattr(self, "_drawer_root_container", None) and self.page.height:
            ...
            # نحدّث الحاوية فوراً هنا (بدل الاعتماد على أن show_drawer سيلتقط هذا
            # التغيير ضمنياً)، لضمان وصول الارتفاع الجديد فعلياً قبل فتح القائمة،
            # حتى لا تُفتح القائمة أحياناً بارتفاعها القديم/الافتراضي (500).
            try:
                self._drawer_root_container.update()
            except Exception:
                pass
    ...
    def build_ui(self):
        ...
        FOOTER_HEIGHT = 40  # ارتفاع تقريبي ثابت لزر "بلازما"
        # مساحة فارغة في آخر القائمة القابلة للتمرير بمقدار ارتفاع الفوتر، حتى لا يُغطّي
        # الزر الثابت (المُثبَّت فوقها عبر Stack) آخر عنصر بالقائمة عند التمرير للأسفل.
        drawer_items.append(ft.Container(height=FOOTER_HEIGHT))
        ...
        # المشكلة لم تكن فقط في حساب الارتفاع (drawer_height/page.height)، بل في طريقة
        # تموضع الزر نفسه: كان "بلازما" يأتي بعد القائمة كعنصر طبيعي ضمن Column، فأي خطأ
        # ولو بسيط بحساب الارتفاع (مثلاً SafeArea يخصم مساحة إضافية فوق ارتفاع محسوب
        # مسبقاً بالفعل من page.height) يدفعه خارج حدود الحاوية بلا أي طريقة للوصول إليه،
        # لأن الـ Column الخارجي نفسه غير قابل للتمرير.
        # الحل الأكثر ثباتاً: تثبيت الزر بشكل مطلق في أسفل الحاوية عبر Stack بدل الاعتماد
        # على تدفّق العناصر الطبيعي (flex layout). هكذا يبقى "بلازما" ملتصقاً بأسفل
        # الحاوية دائماً بغض النظر عن أي خطأ بسيط بحساب الارتفاع، ونضيف padding سفلي
        # للقائمة بمقدار FOOTER_HEIGHT حتى لا يُغطّي الزر آخر عنصر فيها.
        drawer_height = 500
