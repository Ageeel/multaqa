...
class LectureCard(ft.Card):
    ...
    def build_content(self):
        is_finished = str(self.item.get("finished", "false")).lower() in ["true", "1"]
        return ft.Container(
            padding=16,
                        ...
                        controls=[
                            ...
                            ft.Row(
                                controls=[
                                    ...
                                    ft.Text(f"التعليقات: {self.item.get('comments_count', 0)}", size=12, color=ft.colors.BLUE_700)
                                ],
                                spacing=4
                            ) if is_finished else ft.Container(visible=False),
                            ft.IconButton(
...
class TopicCard(ft.Card):
    ...
    def build_content(self):
        ...
        proposal_date = self.item.get("date", "")
